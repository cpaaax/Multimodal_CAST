...
def train(opt, logging):
    ...
    num_train_optimization_steps = int(
        len(train_dataset) / opt.ce_train_batch_size) * opt.max_epochs
    ...
    train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=opt.ce_train_batch_size, num_workers=4)
    ...
    for train_idx in trange(int(opt.max_epochs), desc="Epoch"):
        ...
        logging.info("Epoch | Train Accuracy  | precision score  | recall score  | overall F1  | Training Loss | Validation Loss")
        logging.info(
            f"{train_idx + 1:4d}  |      {train_acc:.4f}      |    {p_score:.4f}     |    {r_score:.4f}      |    {f1_score:.4f}    |    {avg_train_loss:.4f}    |     {avg_val_loss:.4f}")
        print(
             "Epoch | Train Accuracy  | precision score  | recall score  | overall F1    | Training Loss | Validation Loss")
        print(
            f"{train_idx + 1:4d}  |      {train_acc:.4f}      |    {p_score:.4f}     |    {r_score:.4f}      |    {f1_score:.4f}    |    {avg_train_loss:.4f}    |     {avg_val_loss:.4f}")
