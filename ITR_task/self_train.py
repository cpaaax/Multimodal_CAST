...
def self_train(opt,logging):
    logging.info("***********************************************************************************************")
    logging.info("run experiments with seed {} {}".format(opt.seed, opt.loss_w))

    self_train_dataset = load_self_data(opt.text_file_path, opt.self_img_feature_path, opt.comment_file,
                                                      opt.com_num_use, opt.self_train_nums, split='train')
    ...
    ce_num_train_optimization_steps = int(
        len(ce_train_dataset) / opt.ce_train_batch_size) * opt.max_epochs
    self_num_train_optimization_steps = int(
        len(self_train_dataset) / opt.self_train_batch_size) * 5
    optimizer = BertAdam(optimizer_grouped_parameters,
                         lr=opt.learning_rate,
                         warmup=opt.warmup_proportion,
                         t_total=ce_num_train_optimization_steps)
    optimizer_self = BertAdam(optimizer_grouped_parameters,
                            lr=opt.learning_rate,
                            warmup=opt.warmup_proportion,
                            t_total=self_num_train_optimization_steps)
    self_train_dataloader = DataLoader(self_train_dataset, shuffle=True, batch_size=opt.self_train_batch_size, num_workers=4)
    ce_train_dataloader = DataLoader(ce_train_dataset, shuffle=True, batch_size=opt.ce_train_batch_size, num_workers=4)
    dev_dataloader = DataLoader(dev_dataset, shuffle=False, batch_size=opt.ce_train_batch_size, num_workers=4)
    ...
    for train_idx in trange(int(opt.max_epochs), desc="Epoch"):
        ...
        logging.info("  Batch size = %d", opt.ce_train_batch_size)
        ...
        for step, (self_batch, ce_batch) in enumerate(tqdm(zip(self_train_dataloader, ce_train_dataloader), desc="Iteration", total=len(self_train_dataloader))):
        ...
        p_score, r_score, f1_score = utils.calculate_score_all(predict_all, label_all)
        ...
        print()
        logging.info(
            "Epoch | precision score  | recall score  | overall F1  | Validation Loss")
        logging.info(
            f"{train_idx + 1:4d} |    {p_score:.4f}     |    {r_score:.4f}      |    {f1_score:.4f}     |     {avg_val_loss:.4f}")
        print(
            "Epoch | precision score  | recall score  | overall F1  | Validation Loss")
        print(
            f"{train_idx + 1:4d} |    {p_score:.4f}     |    {r_score:.4f}      |    {f1_score:.4f}     |     {avg_val_loss:.4f}")
        ...
        model_save_path = os.path.join(opt.self_save_path)
        if not os.path.exists(model_save_path):
            os.makedirs(model_save_path)
        # torch.save(model.state_dict(), os.path.join(opt.ce_save_path, 'model.pth'))
        # save the best model
        if f1_score > best_val_score:
            torch.save(model_s.state_dict(), os.path.join(model_save_path, 'model_best_{}.pth'.format(opt.ts_iteration)))
